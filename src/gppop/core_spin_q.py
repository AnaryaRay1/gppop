...
class Post_Proc_Utils_spins_with_q(Utils_spins_with_q):
    ...
    def get_pearson_coeff_mass_range_with_q(self, n_corr, dm1, dq, dchi, log_bin_centers, m_min, m_max):
        ...
        for hsamp in range(len(rate_density_array)):
            hyp_samp1 = self.construct_1dtond_matrix_small(len(mbin_chosen), m_min, m_max, rate_density_array[hsamp] * delta_ms * delta_qs * delta_chis,nbins_chi=nbins_chi, nbins_q = nbins_q)
            P_qchi = np.mean(hyp_samp1, axis = 0) / np.sum(np.mean(hyp_samp1, axis = 0))
    ...
    def get_pearson_coeff_mass_marg_with_q(self, n_corr, dm1, dq, dchi, log_bin_centers):
    	...
    	for hsamp in range(len(n_corr)):
            hyp_samp1 = self.construct_1dtond_matrix(nbins_m,n_corr[hsamp] * dm1 * dq * dchi,nbins_chi=nbins_chi, nbins_q = nbins_q)
    ...
    def get_pearson_coeff_chi_range_with_q(self, n_corr, dm1, dq, dchi, log_bin_centers, chi_min, chi_max):
        ...
        for hsamp in range(len(n_corr)):
            hyp_samp1 = self.construct_1dtond_matrix(nbins_m, rate_density_array[hsamp] * delta_ms * delta_qs * delta_chis,nbins_chi=len(chi_bin_chosen), nbins_q = nbins_q)
        
            P_mq = np.mean(hyp_samp1, axis = -1) / np.sum(np.mean(hyp_samp1, axis = -1))
        
            M, Q = np.meshgrid(m1_bin_centers, qbin_centers, indexing='ij')
            E_m = np.sum(M * P_mq)
            E_q = np.sum(Q * P_mq)
            
            E_q2 = np.sum(Q**2 * P_mq)
            E_m2 = np.sum(M**2 * P_mq)
            E_qm = np.sum(M * Q * P_mq)
            ...
            # Step 5: Pearson correlation coefficient
            rho_chi_range.append(Cov_mq / np.sqrt(Var_m * Var_q))
        
        rho_chi_range = np.array(rho_chi_range)
    ...
    def get_pearson_coeff_chi_marg_with_q(self, n_corr, dm1, dq, dchi, log_bin_centers):
        ...
        for hsamp in range(len(n_corr)):
            ...
            hyp_samp_mean = np.mean(hyp_samp1, axis = -1)
            P_mq = hyp_samp_mean / np.sum(hyp_samp_mean)
            ...
            # Step 5: Pearson correlation coefficient
            rho_chi_full.append(Cov_mq / np.sqrt(Var_m * Var_q))
        
        rho_chi_full = np.array(rho_chi_full)
    ...
    def get_two_d_q_chi_with_q(self, n_corr, dm1, m_min, m_max, log_bin_centers):
        ...
        p_avg = self.construct_1dtond_matrix_small(len(mbin_chosen), m_min, m_max, p_avg_m1qchi * delta_ms,nbins_chi=nbins_chi, nbins_q = nbins_q)
        #print(p_avg)
