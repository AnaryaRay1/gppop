...
class Post_Proc_Utils_spins(Utils_spins):
    ...
    def get_Rpchi_m1m2(self,log_bin_centers,n_corr_samples,chi_bins,dm1,dm2,m1_min,m1_max, m2_min, m2_max):
        '''
        ...
        for i in range(nbins_chi):
            idx_array = np.arange(len(log_bin_centers))
            bin_idx = idx_array[(((log_bin_centers[:,0]<=np.log(m1_max))&
                                (log_bin_centers[:,0]>=np.log(m1_min)))&
                                 ((log_bin_centers[:,1]<=np.log(m2_max))&
                                (log_bin_centers[:,1]>=np.log(m2_min))))&
    ...
    def get_Rpchi_m1cm2(self,log_bin_centers,n_corr_samples,chi_bins,dm1,dm2,m1_min,m1_max, m2_min, m2_max):
        '''
        Function for computing p(chi_eff|m1,m2) for either m1 or m2 values belonging in 
        some range.
...
class Rates_spins(Utils_spins):
    ...
    def make_significant_model_3d_n_eff_opt(self,log_bin_centers,weights,tril_vts,tril_deltaLogbins, ls_mean_m, ls_sd_m,ls_mean_chi, ls_sd_chi,sigma_sd=1.,mu_dim=None,vt_sigmas=None, variance_cut=True, wt_means=None, wt_sigmas=None, exponent = -30, maximum_uncertainty = 1.0):
        '''
