...
class Rates_spins(Utils_spins):
    ...
    def make_significant_model_3d_n_eff_opt(self,log_bin_centers,weights,tril_vts,tril_deltaLogbins, ls_mean_m, ls_sd_m,ls_mean_chi, ls_sd_chi,sigma_sd=1.,mu_dim=None,vt_sigmas=None,vt_accuracy_check=False, wt_means=None, wt_sigmas=None, exponent = -30):
        '''
        Function that creates a pymc model that will sample the posterior in 
        Eq. A6 (or B11 if vt_accuracy_check=True) of https://arxiv.org/abs/2304.08046
        for the correlated population model in Eq. 2 and the GP prior in Eq. 5.
