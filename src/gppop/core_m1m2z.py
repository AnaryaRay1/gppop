...
class Utils():
    ...
    def compute_weights(self,samples,m1m2_given_z_prior=None,full_prior=None,O4_prior = False):
        '''
        ...
        wgt_means = np.zeros([len(self.mbins)-1,len(self.mbins)-1,len(self.zbins)-1])
        wgt_sigmas = np.zeros([len(self.mbins)-1,len(self.mbins)-1,len(self.zbins)-1])
...
class Post_Proc_Utils(Utils):
    ...
    def get_Rpm2_corr(self,n_corr,delta_logm1_array,m2_bins,log_bin_centers,z_low,z_high):
        ...
        for i in range(len(m2_bins)-1):
                ...
                rate_density_array = n_corr[:,bin_idx]
                delta_logm1s = delta_logm1_array[bin_idx]
                Rpm2 = np.concatenate((Rpm2,(np.sum(rate_density_array*((delta_logm1s)[None,:]),axis=1)[:,None]/(m_array[None,:]))), axis = 1)
                mass2 = np.append(mass2,m_array)
        return mass2,Rpm2[:,1:]
...
class Rates(Utils):
    ...
    def make_significant_model_3d_n_eff_opt(self,log_bin_centers,weights,tril_vts,tril_deltaLogbins, ls_mean_m, ls_sd_m,ls_mean_z, ls_sd_z,sigma_sd=10,mu_dim=None,vt_sigmas=None,vt_accuracy_check=False, wt_means=None, wt_sigmas=None, exponent = -30):
        '''
        Function that creates a pymc model that will sample the posterior in 
        Eq. A6 (or B11 if vt_accuracy_check=True) of https://arxiv.org/abs/2304.08046
        for the correlated population model in Eq. 2 and the GP prior in Eq. 5.
        ...
        wt_means                        ::    numpy.ndarray
                                               ...
                                               Second output of Utils.compute_weights. Default is 
